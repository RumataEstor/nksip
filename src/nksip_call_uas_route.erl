...
route_reply(Reply, #trans{status=route}=UAS, Call) ->
    ...
    case Route of
        {process, _} when Method=/='CANCEL', Method=/='ACK' ->
            case unsupported_extensions(required_extensions(UAS1), Call1) of
                [] -> 
                    do_route(Route, UAS1, Call1);
                Unsupported ->
                    reply({bad_extension, nksip_lib:bjoin(Unsupported)}, UAS1, Call1)
            end;
        _ ->
            do_route(Route, UAS1, Call1)
    end;
...
required_extensions(#trans{request=Req}) ->
    [Option || {Option, _} <- nksip_sipmsg:header(Req, <<"Require">>, tokens)].


unsupported_extensions([], _) ->
    [];
unsupported_extensions(Required, #call{app_id=AppId, opts=#call_opts{app_module=Module}}) ->
    case nksip_sipapp_srv:sipapp_call(AppId, Module, supported, [], []) of
        {reply, Supported} when is_list(Supported) ->
            Required -- Supported;
        _ ->
            Required
    end.
...
do_process('NOTIFY', DialogId, UAS, Call) ->
    case DialogId of
        <<>> -> reply(no_transaction, UAS, Call);
        _ -> do_process_call(notify, [DialogId], UAS, Call)
    end;
...
-spec do_process_call(atom(), list(), nksip_call:trans(), nksip_call:call()) ->
    nksip_call:call().

do_process_call(Fun, Args, UAS, Call) ->
    #trans{request=#sipmsg{id=ReqId}, method=Method} = UAS,
    #call{opts=#call_opts{app_opts=Opts}} = Call,
    case app_call(Fun, Args, UAS, Call) of
